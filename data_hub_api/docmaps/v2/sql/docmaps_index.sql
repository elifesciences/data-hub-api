...
t_related_article_lookup AS (
  SELECT 
    DISTINCT
    related_article.related_article_id AS rp_manuscript_id,
    article_id AS manuscript_id,
    article_type AS manuscript_type,
    all_version.Manuscript_Title AS manuscript_title,
    ARRAY_TO_STRING( 
      ARRAY(
        SELECT
          authors.Name
        FROM UNNEST(all_version.Authors) AS authors
        ORDER BY authors.Position
      ), ', '
    ) AS manuscript_authors_csv
  FROM `elife-data-pipeline.prod.v_elife_article_xml_data` AS article_xml
  LEFT JOIN UNNEST(related_article_detail) AS related_article
  LEFT JOIN `elife-data-pipeline.prod.mv_Editorial_All_Manuscript_Version` AS all_version
    ON article_xml.article_id = all_version.Manuscript_ID
    AND all_version.Position_In_Overall_Stage = 1
),

t_latest_collections_detail AS (
  SELECT
    _content.id AS content_id,
    collection_id,
    collection_title,
    collection_url,
    colection_curator_name,
    is_colection_curator_et_al
  FROM (
    SELECT
      id AS collection_id,
      title As collection_title,
      image.thumbnail.uri AS collection_url,
      selectedCurator.name.preferred AS colection_curator_name,
      selectedCurator.etAl AS is_colection_curator_et_al,
      content,
      ROW_NUMBER() OVER (
        PARTITION BY id
        ORDER BY imported_timestamp
      ) AS rn
    FROM `elife-data-pipeline.prod.elife_collections_detail`
  ) AS collections
  LEFT JOIN UNNEST(collections.content) AS _content
),

t_result_with_related_content AS (
  SELECT
    result.*,
    ARRAY(
      SELECT AS STRUCT 
      related_article.* EXCEPT(rp_manuscript_id),
      collections.* EXCEPT(content_id)
    ) AS related_content
    
  FROM t_result_with_sorted_manuscript_versions_array AS result
  LEFT JOIN t_related_article_lookup AS related_article
    ON result.manuscript_id = related_article.rp_manuscript_id
  LEFT JOIN t_latest_collections_detail AS collections
    ON result.manuscript_id = collections.content_id
),
...
FROM t_result_with_related_content AS result
